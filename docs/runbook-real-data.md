...
| `tags` column (LLM vibe/specialty tags) | same `places` rows | `npm run enrich` (local `claude` CLI) |
...
- Accounts you'll create below: **Supabase**, **Google Cloud**.
- The **`claude` CLI** installed and logged in to your Claude subscription
  (run `claude` once interactively to confirm). Enrichment shells out to it;
  it is **not** the Anthropic API and needs **no `ANTHROPIC_API_KEY`**.
...
## 3. Enrichment LLM: the local `claude` CLI (no API key)

Enrichment tags are generated by the **`claude` CLI running on your Claude
subscription** — `scripts/enrichPlaces.ts` shells out to `claude -p`. There is
**no Anthropic API key and no per-token billing.**

> ⚠️ **Do NOT create or set `ANTHROPIC_API_KEY` for this project.** The enrich
> script deliberately deletes that variable at startup and strips it from the
> `claude` child process, so the Anthropic API can never be billed. An API key
> once leaked in from the shell environment and cost ~$20 before this was
> hard-wired shut; keep it out.

1. Make sure `claude` is installed and logged in (`claude` interactively, once).
2. That's it — no key to copy, nothing to add to `.env` for the LLM step.
...
### LLM enrichment: nothing to cap (subscription, not the API)

Enrichment runs through the local `claude` CLI on your Claude subscription, so
there is **no metered API spend to cap** — no Anthropic billing is involved at
all. The only limit is your plan's normal rolling usage window; if a large batch
hits it, the run skips the rest and a later `npm run enrich` retries only the
still-untagged rows. Do **not** add an `ANTHROPIC_API_KEY` to work around this.
...
# No ANTHROPIC_API_KEY: enrichment uses the local `claude` CLI (see step 3).
```
...
npm run enrich         # local `claude` CLI tags -> tags column (only untagged rows)
```
...
| `enrich` | ~300 `claude` CLI calls | your Claude subscription | $0 (no metered API) |

The ~$9 of Google usage sits inside Google's free monthly allowance, and the LLM
tagging runs on your Claude subscription (no per-token cost), so real
out-of-pocket for a seed is **~$0**. **Ongoing** cost
...
- **LLM tagging:** ~1 short `claude` CLI call per untagged place, on your
  Claude subscription — no Anthropic API, no per-token billing.
